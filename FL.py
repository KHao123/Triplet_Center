...
from focalloss2d import FocalLoss
...
if __name__ == '__main__':
	...
	if args.EmbeddingMode:
		...
		loss_fn = FocalLoss(class_num = n_classes, gamma = 2)
		# if dataset_name == 'skin':
		# 	loss_fn = torch.nn.CrossEntropyLoss(torch.FloatTensor([0.036,0.002,0.084,0.134,0.037,0.391,0.316]).cuda()) 
		lr = 1e-4
		# optimizer = optim.Adam(model.parameters(), lr=lr)
		optimizer = optim.Adam(
                    model.parameters(),
		...
		logName = '{}d-focalloss'.format(num_of_dim)
		logName = os.path.join(Attr_Dict[dataset_name]['resDir'],logName)
		EmbeddingArgs = ()
