...
if __name__ == '__main__':
	...
	if args.EmbeddingMode:
		...
		weight_ = torch.FloatTensor([0.01317614, 0.87021505, 0.11660882]).cuda()
		loss_fn = FocalLoss(class_num = n_classes, alpha = weight_, gamma = 2)
		lr = 1e-4
		optimizer = optim.Adam(
                    model.parameters(),
                    lr=lr,
		...
		logName = '{}d-weight-focalloss'.format(num_of_dim)
		logName = os.path.join(Attr_Dict[dataset_name]['resDir'],logName)
		EmbeddingArgs = ()
