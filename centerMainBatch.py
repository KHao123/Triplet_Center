...
from CenterLoss import CenterLoss , calc_centers
import torch
...
parser.add_argument('--dataset_name',default='skin',
				help='Choose dataset [skin,MNIST,...]')
parser.add_argument('--rescale',default=False,type=str2bool,help='rescale dataset')
parser.add_argument('--iterNo',default=1,type=int,help='Use for choosing skin-image set')
parser.add_argument('--cuda_device',default=0,type=int,help='Choose cuda_device:(0,1,2,3)')
parser.add_argument('--EmbeddingMode',default=True,type = str2bool ,
...
parser.add_argument('--start_epoch',default=0,type=int,help='Start epoch (int)')
parser.add_argument('--n_epoch',default=200,type=int,help='End_epoch (int)')
parser.add_argument('--batch_size',default=256,type=int,help='Batch size (int)')
parser.add_argument('--probability',default=400./5000.,type=float,help='Leave out probability')
parser.add_argument('--mnist1',default=5,type=int,help='Sampling num1 with probability')
parser.add_argument('--mnist2',default=8,type=int,help='Sampling num2 with probability')

parser.add_argument('--n_sample_classes',default=10,type=int,
				help='For a batch sampler to work comine #samples_per_class')
parser.add_argument('--n_samples_per_class',default=25,type=int,
...
def calc_centers_whole(dataloader,model,n_classes):
	with torch.no_grad():
		model.eval()
...
if __name__ == '__main__':
	...
	train_dataset = skinDatasetFolder(train=True,iterNo=args.iterNo,data_dir='../data')
	test_dataset = skinDatasetFolder(train=False,iterNo=args.iterNo,data_dir='../data')
	...
	train_batch_sampler = BalancedBatchSampler(train_dataset, n_classes=args.n_sample_classes, n_samples=args.n_samples_per_class)
	test_batch_sampler = BalancedBatchSampler(test_dataset,  n_classes=args.n_sample_classes, n_samples=args.n_sample_classes)
	...
	output_writer_path = os.path.join('./run', '{}/iter{}/CenterlossBatch_margin{}_{}d'.format(dataset_name,args.iterNo,margin,args.dim))
	...
	for epoch in range(start_epoch,n_epochs):
		...
		for batch_idx,(data,target) in enumerate(online_train_loader):
			...
			centers = calc_centers(batch_embedding,target,n_classes)
			centers.requires_grad_()
		...
		with torch.no_grad():
			...
			correct_test = []
			pred_test = []
			centers = calc_centers_whole(train_loader,model,n_classes)
